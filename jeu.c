...
{
    SDL_Surface *tutoriel=NULL;
    SDL_Surface *histoire_intro=NULL;
    SDL_Surface *Loading[4];
    TTF_Font *police=TTF_OpenFont("resources/polices/Enchanted Land.otf",70);
    SDL_Color couleur= {255,255,255};
    SDL_Surface *fond_ecran=IMG_Load("resources/images/test.jpg");

    Mix_Music  *son_intro=NULL;
    son_intro=Mix_LoadMUS("resources/sons/son_pregame.mp3");
    Mix_PlayMusic(son_intro,1);


    Loading[0]=TTF_RenderText_Blended(police,"LOADING ",couleur);
    Loading[1]=TTF_RenderText_Blended(police,"LOADING. ",couleur);
    Loading[2]=TTF_RenderText_Blended(police,"LOADING.. ",couleur);
    Loading[3]=TTF_RenderText_Blended(police,"LOADING...",couleur);

    SDL_Rect position;
    position.x=0;
    position.y=0;

    SDL_Rect position_loading;
    position_loading.x=ecran->w/3;
    position_loading.y=ecran->h/2;


    switch(mode_de_jeu)
    {
    case mono_joueur:
        tutoriel=IMG_Load("resources/images/tutorial_monojoueur.png");
        histoire_intro=IMG_Load("resources/images/gamehistory.png");
        break;
    case multi_joueur:
        tutoriel=IMG_Load("resources/images/tutorial_multijoueur.png");
        histoire_intro=IMG_Load("resources/images/gamehistory.png");
        break;
    }

    SDL_FillRect(ecran,NULL,SDL_MapRGB(ecran->format,0,0,0));
    SDL_BlitSurface(histoire_intro,NULL,ecran,&position);
    SDL_Flip(ecran);
    SDL_Delay(5000);

    SDL_FillRect(ecran,NULL,SDL_MapRGB(ecran->format,0,0,0));
    SDL_BlitSurface(tutoriel,NULL,ecran,&position);
    SDL_Flip(ecran);
    SDL_Delay(5000);
    ...
    for(i=0; i<4; i++)
    {
        ...
        SDL_BlitSurface(Loading[i],NULL,ecran,&position_loading);
        SDL_Flip(ecran);
        SDL_Delay(500);
    }
    for(i=0; i<4; i++)
    ...
    SDL_FreeSurface(tutoriel);
    SDL_FreeSurface(histoire_intro);
    ...
    Mix_FreeMusic(son_intro);
}

int collision_pieces(SDL_Rect pos_pieces,Personnage p)
{
    if ((p.position.x + p.position_sprite.w< pos_pieces.x)||(p.position.x> pos_pieces.x + pos_pieces.w) || (p.position.y + p.position_sprite.h< pos_pieces.y)||(p.position.y> pos_pieces.y + pos_pieces.h))
        return 0;
    else
        return 1;
}
...
////////////////////////////
        ...
        if(joueur1.nombre_vie==0)
        {
            ...
            if(mise_a_jour_highscores("resources/images/highestscores.txt",joueur1.score)==1)
            {
                SDL_FillRect(ecran,NULL,SDL_MapRGB(ecran->format,0,0,0));
                SDL_BlitSurface(IMG_Load("resources/sauvegardes/new_highest_score.png"),NULL,ecran,&fond_ecran_niv1.pos_background1);
                SDL_Flip(ecran);
                SDL_Delay(3000);
            }
...
int niveau2_mono_joueur(SDL_Surface *ecran)
{
    ...
    load_transition(&joueur1,&fond_ecran_niv1,&time,&niv);
...
//lancement musique jeu niveau1
    ...
    while(continuer)
    {
        ...
        case SDL_KEYDOWN:
            ...
            {
                if((ennemi_present==1 || pieces_presente==1 || pieces_special_presente==1 || vies_presente==1 || niveau_termine==1))
                {
                    joueur1.action=marcher_droite;
                    joueur1.direction=droite;
                    if(joueur1.position.x<ecran->w-60)
